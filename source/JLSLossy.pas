  ...
  begin
    ...
    repeat
      ...
      begin
        ...
        if ( delta <= FImageInfo^.NEAR) and (delta >= FImageInfo^.negNEAR ) then
        begin
          ...
          begin
            ...
            delta := Ix-Ra;
            if ( delta >  FImageInfo^.NEAR) or (delta < FImageInfo^.negNEAR ) then { Run is broken }
            begin
              Rd := psl^[i + 1];
              Rb := psl^[i];
        ...
        if  ((Rb-Ra)<=FImageInfo^.NEAR) and ((Rb-Ra)>=FImageInfo^.negNEAR) then
          RItype := 1
        else
          RItype := 0;
    ...
    repeat
      ...
      begin
        ...
        begin
          ...
          if (delta[n_c]>FImageInfo^.NEAR) or (delta[n_c]<FImageInfo^.negNEAR) then enter_run:=0;
        end;
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              if(delta[n_c]>FImageInfo^.NEAR) or (delta[n_c]<FImageInfo^.negNEAR) then break_run:=1;
            end;
...
begin
  ...
  if ((k=0) and (FImageInfo^.NEAR=0) and (2*Bt <= -Nt) ) then
  begin
...
begin
  ...
  { Error Mapping (A.5.2) }
  temp := Bool_c( (k=0) and (FImageInfo^.NEAR=0) and ((Bt shl 1) <= -Nt) );
  if (qErrval >= FImageInfo^.ceil_half_qbeta) then
  begin
    qErrval := qErrval - FImageInfo^.qbeta;
    absErrval := -qErrval;
  ...
  begin
    ...
    repeat
      ...
      {********** RUN STATE *********}
        ...
        if ( (Rb-Ra) <= FImageInfo^.NEAR) and ((Rb-Ra) >= FImageInfo^.negNEAR) then
        run_int_type := 1 else run_int_type := 0;
    ...
    repeat
      ...
      if (color = 0) then
        ...
        begin
          ...
          begin
            if (diff > -LUTMAX16)
              then c_cont[n_c] :=  FImageInfo^.vLUT[0][diff + LUTMAX16]
              else c_cont[n_c] :=  7 * CREGIONS * CREGIONS;
          end
          else
          begin
            if (diff < LUTMAX16)
              then c_cont[n_c] := FImageInfo^.vLUT[0][diff + LUTMAX16]
              else c_cont[n_c] := 8 * CREGIONS * CREGIONS;
          end;
          ...
          begin
            if (diff > -LUTMAX16)
              then c_cont[n_c] := c_cont[n_c] + FImageInfo^.vLUT[1][diff + LUTMAX16]
              else c_cont[n_c] := c_cont[n_c] + 7 * CREGIONS;
          end
          else
          begin
            if (diff < LUTMAX16) then  c_cont[n_c] :=  c_cont[n_c] + FImageInfo^.vLUT[1][diff + LUTMAX16]
            else c_cont[n_c] := c_cont[n_c] +8*CREGIONS;
          end;
          ...
          begin
           if (diff > -LUTMAX16) then
             c_cont[n_c] :=  c_cont[n_c] + FImageInfo^.vLUT[2][diff + LUTMAX16]
             else  c_cont[n_c] :=  c_cont[n_c] + 7;
          end
          else
          begin
           if (diff < LUTMAX16)
             then c_cont[n_c] :=  c_cont[n_c] + FImageInfo^.vLUT[2][diff + LUTMAX16]
             else c_cont[n_c] :=  c_cont[n_c] + 8;
          end;
